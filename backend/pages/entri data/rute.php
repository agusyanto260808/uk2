...
// Ambil data rute + transportasi + semua jadwal (berangkat & tiba)
$q = mysqli_query($conn, "
    SELECT 
        r.id_rute,
        r.tujuan,
        ...
        r.rute_ahir,
        r.harga,
        t.kode AS transportasi,
        GROUP_CONCAT(
            DISTINCT CONCAT(
                TIME_FORMAT(j.jam_berangkat, '%H:%i'),
                ' → ',
                TIME_FORMAT(j.jam_tiba, '%H:%i')
            )
            ORDER BY j.jam_berangkat ASC SEPARATOR ', '
        ) AS jadwal
    FROM rute r
    JOIN transportasi t ON r.id_transportasi = t.id_transportasi
    LEFT JOIN jadwal j ON r.id_rute = j.id_rute
    GROUP BY r.id_rute
");
    ...
    <div class="page-inner mt-4 px-3">
        <div class="card shadow border-0">
            <div class="card-header d-flex justify-content-between align-items-center bg-primary text-white">
                <h5 class="mb-0">Data Rute</h5>
                <a href="../entri data/create_rute.php" class="btn btn-light btn-sm">+ Tambah</a>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-bordered align-middle datatable">
                        ...
                        <tbody>
                            <?php if (mysqli_num_rows($q) > 0): ?>
                                ...
                                while ($row = mysqli_fetch_assoc($q)): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($row['rute_ahir']); ?></td>
                                        <td class="text-end">Rp <?= number_format($row['harga'], 0, ',', '.'); ?></td>
                                        <td class="text-center"><?= htmlspecialchars($row['transportasi']); ?></td>
                                        ...
                                        <td class="text-center">
                                            <a href="../entri data/edit_rute.php?id_rute=<?= $row['id_rute']; ?>"
                                                class="btn btn-warning btn-sm">
                                                Edit
                                            </a>
                                            <a href="../../actions/entri data/delete_rute.php?id_rute=<?= $row['id_rute']; ?>"
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-4">Belum ada data rute.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
